module github.com/ODIM-Project/ODIM/svc-aggregation

go 1.13

require (
	github.com/ODIM-Project/ODIM/lib-dmtf v0.0.0-20200727115727-33d557ff397c
	github.com/ODIM-Project/ODIM/lib-messagebus v0.0.0-20201007095715-369040919a43
	github.com/ODIM-Project/ODIM/lib-rest-client v0.0.0-20201007095715-369040919a43
	github.com/ODIM-Project/ODIM/lib-utilities v0.0.0-20200925145026-eac0549d2f51
	github.com/satori/go.uuid v1.2.0
	github.com/stretchr/testify v1.5.1
)
